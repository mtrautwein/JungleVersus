...
local searching_server = false
local connected_to_server = false
...
local red = {x=120, y=300, startX=120, startY=300, w=30, h=48, vx=0, vy=0, speed=300, jump=false, shoot={liste={},bool=false,speed=600,size=15,recul=6,damage=7,gravity=4}, state='right', sante=100, round_gagne=0, choice=nil} 

local blue = {x=660, y=300, startX=660, startY=300, w=30, h=48, vx=0, vy=0, speed=300, jump=false, shoot={liste={},bool=false,speed=600,size=15,recul=6,damage=7,gravity=4}, state='left', sante=100, round_gagne=0, choice=1}
...
function sound.load()
	...
	snd_jump = sound.new('jump',2,0.25)
	snd_hit = sound.new('hit',2,0.3)
	snd_hurt = sound.new('hurt',2,0.3)
...
function animation.play(pAnim_table, nombre_de_fois,dt)
	if pAnim_table.bool then 
		pAnim_table.timer = pAnim_table.timer + pAnim_table.vitesse*dt
		if math.floor(pAnim_table.timer) > pAnim_table.maxTimer then 
			pAnim_table.timer = 1
...
function animation.draw()
	--draw
	love.graphics.draw(anim_flecheJoueur.img, anim_flecheJoueur[math.floor(anim_flecheJoueur.timer)], blue.x+blue.w/2-5, blue.y - 17,0,scale,scale)
end	
---

-- TIRS --
function tirs.create(player)
	local tir = {y=player.y, vx=0, vy=0, w=player.shoot.size, h=player.shoot.size, direction=player.state,img=player.shoot.img}

	if player==red then tir.color={178,89,72} else tir.color={67,109,118} end 
...
function players.movements_red(dt)
	...
	if rx ~= 0 or ry ~= 0 then
   		red.x, red.y, cols_len = world:move(red, red.x + rx, red.y + ry)
    end
end	

function players.movements_red_2(dt)
	if data then 
		local p = split(data,'-')
		...
		if tonumber(p[1]) ~= nil and tonumber(p[2]) ~= nil then 
			red.x, red.y = tonumber(p[1]), tonumber(p[2])	
		end

		red.x, red.y, cols_len = world:move(red, red.x, red.y)
	end	
end	

function players.movements_blue(dt)
	-- BLUE movements // send to SERVER
	local bx, by = 0, 0 	 
	...
	-- gestion des tirs 
	--blue.shoot.timer = blue.shoot.timer - blue.shoot.cadence*dt
	--if love.keyboard.isDown('space') and is_inside(blue) and blue.shoot.timer <= 0 then 
	--	tirs.create(blue)
	--	shoot_state = 's'
...
function players.draw()
	--setColor(178,89,72)
	--love.graphics.rectangle("fill", red.x, red.y, red.w, red.h)
	--setColor(67,109,118)
...
function game.keypressed(key)
	if key == 'up' and blue.jump == false then 
	...
	if key == 'space' and is_inside(blue) then 
		tirs.create(blue)
		if blue.state == 'right' then 
			blue.vx = -blue.shoot.recul
		elseif blue.state == 'left' then 
...
function menu.update_connection(dt)
    ...
    if searching_server == true and timer_connection <= 0 then 
	    udp:send('Requête de connection au serveur.')
	    if data then 
...
function menu.keypressed_selection(key)
	if key == 'right' and blue.choice < 3 then 
		blue.choice = blue.choice + 1
	elseif key == 'left' and blue.choice > 1 then 
		blue.choice = blue.choice - 1	
	end	
...
function menu.update_synchro(dt)
    if blue.choice == 1 then
		udp:send('chara_big')
	elseif blue.choice == 2 then 
		udp:send('chara_middle')
	elseif blue.choice == 3 then 
		udp:send('chara_small')
	end
...
-- MAIN FUNCTIONS --
function love.load()
	love.window.setTitle('JungleVersus / CLIENT')
...
function love.update(dt)
    -- DATA reception from the server
    data = udp:receive()	
    ...
    --if update_timer > update_rate then -- Pour ne pas saturer le serveur/client 
    	--udp:send(tostring(blue.x+blue.vx)..'-'..tostring(blue.y+blue.vy)..'-'..tostring(walk_state)..'-'..shoot_state)
    	--udp:send(blue.state..'-'..jump_state..'-'..shoot_state)		
...
function love.draw()
	...
	setColor(0,0,0)
	if red.x ~= nil and red.y ~= nil then 
		--love.graphics.print('red X: '..red.x..'    red Y : '..red.y,10,10)
	else 
		--love.graphics.print('red X: nil    red Y : nil',10,10)
	end	
	setColor()
end	
